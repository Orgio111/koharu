...
impl Xpos {
    ...
    fn forward(&self, xs: &Tensor, offset: usize, downscale: bool) -> Result<Tensor> {
        ...
        let (mut sin, mut cos) = fixed_pos_embedding(scale.dims2()?.0, half_dim, xs.device(), xs.dtype())?;
...
fn fixed_pos_embedding(seq_len: usize, dim: usize, device: &Device, dtype: DType) -> Result<(Tensor, Tensor)> {
